...
def parse_link(uri):
    ...
    if uri.startswith('vmess://'):
        try:
            raw = base64.b64decode(_b64_pad(uri[8:]), validate=False).decode('utf-8', 'replace')
            cfg = json.loads(raw)
...
def verify(binary, spec, ip, socks_port, port=None, timeout=8.0, want_speed=False):
    ...
    try:
        with XrayTunnel(binary, spec, ip, socks_port, port):
            status, _, _ = _https_get(socks_port, PROBE_HOST, PROBE_PATH, timeout)
            handshake_ms = int((time.monotonic() - started) * 1000)
            if status != PROBE_EXPECT and not (200 <= status < 400):
                return {'ok': False, 'error': f'tunnel HTTP {status}'}
